...
//     QuillEditor,
//     axios,
//     swal, // Daftarkan komponen QuillEditor
//   },
//   mounted(){
...
const fetchData = async () => {
  try {
    ...
    data.judul = dataArticle.value[0].judul
    data.penulis = dataArticle.value[0].penulis
    data.kategori = dataArticle.value[0].kategori
...
const EditArtikel = async () => {
      const fetchData = async () => {
        try {
          ...
          //   console.log("ini error :", error);
          // });
          swal("Sukses", "Sukses Update Article", "success");
          this.data = null;
          //   console.log(data);
          console.log(response);
        } catch (error) {
...
<template>
  <!-- <div v-for="artikel in dataArticle" :key="artikel.id"> -->
    ...
    <div class="w-[50%]">
      <div class="relative h-10 w-full min-w-[200px]">
        <input v-model="dataArticle" type="hidden"/>
        <input v-model="dataArticle" type="hidden"/>
        <input v-model="dataArticle" type="hidden"/>
        <input
    ...
    <div class="w-[50%] mt-[-30px]">
      ...
      <QuillEditor  :initial-value="data.konten" @update:editorContent="updateEditorContent"  />
    </div>
